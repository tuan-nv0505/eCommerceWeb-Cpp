...
template <size_t... Is>
inline drogon::Task<drogon::orm::Result>
drogon_exec_coro_impl(const std::shared_ptr<drogon::orm::DbClient> &dbClient,
...
inline drogon::Task<drogon::orm::Result>
drogon_co_bind(const std::shared_ptr<drogon::orm::DbClient> &dbClient,
               const std::string &sql,
               const std::vector<std::string> &bindValues) {
  if (bindValues.size() > MaxArgs) {
    throw std::out_of_range("Can only bind up to " + std::to_string(MaxArgs) +
                            " values");
  }

  using ReturnType = drogon::Task<drogon::orm::Result>;
  using FnPtr =
      ReturnType (*)(const std::shared_ptr<drogon::orm::DbClient> &,
  ...
  static constexpr auto table = []<size_t... Is>(std::index_sequence<Is...>) {
    return std::array<FnPtr, sizeof...(Is)>{
        [](const std::shared_ptr<drogon::orm::DbClient> &client,
           const std::string &q, const std::vector<std::string> &v) {
          return drogon_exec_coro_impl(client, q, v,
...
inline std::string build_placeholder_sql(const std::string &baseSql,
                                         size_t count, bool isPostgres = true) {
  if (count == 0) {
    return baseSql + "()";
  }
